import { Badge, Button, Card, Group, Image, Indicator, Text, useMediaQuery } from "@mantine/core";
...
const ProjectCard = (props: any) => {
    ...
    return (
        <div className={`w-[32%] ${isSmallScreen ? 'w-[90%]' : ''} ${isXSmallScreen ? 'w-full' : ''}`} data-aos="fade-up" data-aos-duration="800">
            <Card onClick={open} className="!bg-bgColor cursor-pointer transition-transform duration-300 ease-in-out hover:!scale-[1.02] mb-5 hover:!shadow-[0_0_10px_1px_#64FFDA80] xs-mx:!shadow-[0_0_10px_1px_#64FFDA80] !border-primaryColor border-2" shadow="lg" padding="sm" radius="lg" withBorder>
                ...
                <Group justify="space-between" mt="xs" mb="xs">
                    <div className="!text-2xl gap-2 !font-bold !text-white flex items-center sm-mx:!text-xl">
                        {props.title}
                        {props.live === true && (
                            <Badge className="!px-1" variant="outline" color="red" rightSection={<Indicator className="!mr-0.5 !z-0" color="red" position="middle-end" size={7} processing />} >
                                Live
                            </Badge>
                        )}
                    </div>
                ...
                <Group mb="sm" className="!gap-2">
                    {props.technologies.map((tech: string, index: number) =>
                        index < 3 && <Badge key={index} size={isXSmallScreen ? 'xs' : 'md'} variant="light" color="#64FFDA">{tech}</Badge>
                    )}
                </Group>
